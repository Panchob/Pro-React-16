...
export default class ProductAndSupplier extends Component {
    constructor(props){
        super(props);
        this.state = {
            products: [
    ...
    saveData = (collection, item) => {
        if(item.id === "") {
            ...
            this.setState(state => state[collection] = state[collection].concat(item))
                     ...
                     this.setState(state => state[collection] = state[collection].map(stored =>
                stored.id === item.id ? item : stored))
        }
    }

    deleteData = (collection, item) => {
        this.setState(state => state[collection].filter(stored =>
            stored.id !== item.id));
    }
